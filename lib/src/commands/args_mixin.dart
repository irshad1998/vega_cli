...
mixin ArgsMixin {
  final List<String> _args = VegaCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(VegaCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return VegaCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < VegaCli.arguments.length) {
      index++;
      return VegaCli.arguments[index];
    } else {
